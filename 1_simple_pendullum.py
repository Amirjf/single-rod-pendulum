        ...
        if len(self.wave_x) > 1:
            ...
            for i in range(1, len(self.wave_x)):
                pygame.draw.line(surface, self.color, 
                                 (width - len(self.wave_x) + i - 1, height - 200 + shift + int(self.wave_x[i - 1] / 10)), 
                                 (width - len(self.wave_x) + i, height - 200 + shift + int(self.wave_x[i] / 10)), 2)
...
def run():
    ...
    pendulum = Pendulum(width//2, 200)  # Start pivot at (width/2, 200)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key in [pygame.K_ESCAPE, pygame.K_q]:
                    ...
                    pendulum = Pendulum(width//2, 200)

        # Update and draw the pendulum
        pendulum.update()
        pendulum.plot_trajectory(surface)
        pendulum.draw(surface)

        # Draw the sine wave at the bottom of the screen
        pendulum.plot_sine_wave(surface, width, height, shift=-100)
